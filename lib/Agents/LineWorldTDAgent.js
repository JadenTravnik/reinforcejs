...
var setConst = function(arr, c) {
  for(var i=0,n=arr.length;i<n;i++) {
    ...
    //   probs.push(this.P[poss[i] + obs.s*this.env.getMaxNumActions()]);
    // }


// given the probability matrix
var sampleWeighted = function(p) {
  var argmax = 0, max = -Infinity;
...
var LineWorldTD = function(env, opt) {
...
LineWorldTD.prototype = {
  reset: function(){
    ...
    for(var s=0;s<this.ns;s++) {
      ...
      for(var i=0,n=poss.length;i<n;i++) {
        this.P[poss[i]*this.ns+s] = 1.0 / poss.length; // TODO: state and action calc
      }
    }
  ...
  act: function(obs){
    ...
    var poss = this.env.allowedActions(obs.cell);
  ...
  updateModel: function(s0, a0, r0, s1) {
    ...
    var sa = a0 * this.ns + s0; // TODO: state and action calc
    if(this.env_model_s[sa] === -1) {
      // first time we see this state action
      this.sa_seen.push(a0 * this.ns + s0); // add as seen state TODO: state and action calc
    }
  ...
  learnFromTuple: function(s0, a0, r0, s1, a1, lambda) {
    ...
    if(this.update === 'qlearn') {
      ...
      var s1a1 = a1 * this.ns + s1; // TODO: state and action calc
    ...
    if(lambda > 0) {
      ...
      for(var s=0;s<this.ns;s++) {
        ...
        for(var i=0;i<poss.length;i++) {
          var a = poss[i];
          var saloop = a * this.ns + s; // TODO: state and action calc
  ...
  updatePriority: function(s,a,u) {
    ...
    for(var si=0;si<this.ns;si++) {
      ...
      for(var ai=0;ai<this.na;ai++) {
        var siai = ai * this.ns + si; // TODO: state and action calc
  ...
  updatePolicy: function(s) {
    ...
    for(var i=0,n=poss.length;i<n;i++) {
      var a = poss[i];
      var qval = this.Q[a*this.ns+s]; // TODO: state and action calc
      qs.push(qval);
    ...
    for(var i=0,n=poss.length;i<n;i++) {
      ...
      var ix = a*this.ns+s; // TODO: state and action calc
      if(this.smooth_policy_update) {
        // slightly hacky :p
        this.P[ix] += this.beta * (target - this.P[ix]);
        if (isNaN(this.P[ix])){
          console.log(this.P[ix]);
        }        
        psum += this.P[ix];
      } else {
        // set hard target
    ...
    if(this.smooth_policy_update) {
      ...
      for(var i=0,n=poss.length;i<n;i++) {
        var a = poss[i];
        this.P[a*this.ns+s] /= psum; // TODO: state and action calc
      }
    }
  }
}
...
// exports
global.LineWorldTD = LineWorldTD;
})(RL);
