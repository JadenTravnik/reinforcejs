...
var Agent = function() {
  ...
  this.eyes = [];
  this.num_eyes = 30;
  var eyeAngle = (Math.PI*2)/this.num_eyes;
  console.log("eyeAngle: " + eyeAngle);
  for(var k=0;k<this.num_eyes;k++) { this.eyes.push(new Eye(k*0.21)); }
  ...
  this.last_state_data = Array.apply(null, Array(this.num_states)).map(Number.prototype.valueOf,0);
  this.last_eye_index = 0;  
}
Agent.prototype = {
  ...
  chooseEye: function(){
    this.last_eye_index = (this.last_eye_index == this.num_eyes - 1) ? 0 : this.last_eye_index+1; 
    return this.last_eye_index;
  },
  forward: function() {
    // in forward pass the agent simply behaves in the environment
    // create input to brain
    var ne = this.num_eyes * 5;
    

    // add policy to choose which eye is viewed, use last data for unobserved eyes
    var input_array = new Array(this.num_states);

    // possible acctions


    var eIndex = this.chooseEye();
    var e = this.eyes[eIndex];
    input_array[eIndex*5] = 1.0;
    input_array[eIndex*5+1] = 1.0;
    input_array[eIndex*5+2] = 1.0;
    input_array[eIndex*5+3] = e.vx; // velocity information of the sensed target
    input_array[eIndex*5+4] = e.vy;
    if(e.sensed_type !== -1) {
      // sensed_type is 0 for wall, 1 for food and 2 for poison.
      // lets do a 1-of-k encoding into the input array
      input_array[eIndex*5 + e.sensed_type] = e.sensed_proximity/e.max_range; // normalize to [0,1]
    }
